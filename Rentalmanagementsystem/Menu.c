        ...
        switch (choose)
        {
            ...
            case 1:
                ...
                do
                {
                    ...
                    tempPeople = peopleHead;
                    while (tempPeople->next != NULL)
                    {
                        tempPeople = tempPeople->next;
                    }
                    roomHead = ReadRoomData(roomHead);
                    tempRoom = SmartSearchRoom(roomHead, tempPeople->people->number);
                    printf("请输入入住时间(xxxx-xx-xx):");
                    scanf("%s", tempRoom->room->in);
                    printf("请输入到期时间:(xxxx-xx-xx):");
                    scanf("%s", tempRoom->room->out);
                    tempRoom->room->state = 1;
            ...
            case 3:
                ...
                case 1:
                    printf("【显示所有租客】\n");
                    peopleHead = ReadPeopleData(peopleHead);
                ...
                do
                    {
                        ...
                        roomHead = ReadRoomData(roomHead);
                        tempRoom = SmartSearchRoom(roomHead, tempPeople->people->number);
                        strcpy(tempRoom->room->in, "暂无");
                        strcpy(tempRoom->room->out, "暂无");
                        tempRoom->room->state = 0;
                        UpdateRoomDataManagement(roomHead);
